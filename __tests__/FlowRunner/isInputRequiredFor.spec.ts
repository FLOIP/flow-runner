import {FlowRunner, IBasePromptConfig, INumericPromptConfig, KnownPrompts, TCursor} from '../../src'
...
describe('FlowRunner/isInputRequiredFor', () => {
  ...
  it('should return false when prompt absent', () => {
    expect(runner.isInputRequiredFor({cursor: ['intx-123', undefined]} as IContext)).toBeFalsy()
  })
  ...
  it('should return true when prompt config\'s value is undefined', () => {
    ...
    expect(runner.isInputRequiredFor({cursor: ['intx-123', promptConfig]} as IContext)).toBeTruthy()
  })
  ...
  it('should return false when prompt validation succeeds', () => {
    ...
    jest.spyOn(runner, 'hydrateRichCursorFrom')
      .mockImplementation(() => [
        {uuid: 'intx-123'} as IBlockInteraction,
        createNumericPromptFor(promptConfig, runner)]) // assertion consumes this prompt instance

    expect(runner.isInputRequiredFor({cursor: ['intx-123', promptConfig]} as IContext)).toBeFalsy()
  })
  ...
  it('should return true when prompt validation raises', () => {
    ...
    expect(runner.isInputRequiredFor({cursor: ['intx-123', promptConfig] as TCursor} as IContext)).toBeTruthy()
  })
