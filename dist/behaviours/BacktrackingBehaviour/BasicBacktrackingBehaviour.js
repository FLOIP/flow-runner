...
class BasicBacktrackingBehaviour {
        ...
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            lodash_1.forEachRight(discarded, ({ uuid }) => {
                ...
                while (((_a = lodash_1.last(context.reversibleOperations)) === null || _a === void 0 ? void 0 : _a.interactionId) === uuid) {
                    index_1.FlowRunner.prototype.reverseLastDataOperation(context);
                }
            });
            return this.navigator.navigateTo(index_1.findBlockOnActiveFlowWith(intx.blockId, context), context);
        });
    }
    _findInteractiveInteractionAt(steps = 0, context = this.context, direction = PeekDirection.LEFT) {
        ...
        if (_find == null) {
            throw new index_1.ValidationException(`Unknown \`direction\` provided to findInteractiveInteractionAt() -
        ${JSON.stringify(direction)}`);
        }
        let _steps = steps + 1;
        const intx = _find(context.interactions, ({ type }) => !lodash_1.includes(index_1.NON_INTERACTIVE_BLOCK_TYPES, type) && --_steps === 0);
        if (intx == null || _steps > 0) {
            throw new index_1.ValidationException(`Unable to backtrack to an interaction that far back ${JSON.stringify({ steps })}`);
        }
        return intx;
    }
    peek(steps = 0, context = this.context, direction = PeekDirection.LEFT) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            const block = index_1.findBlockWith(intx.blockId, index_1.findFlowWith(intx.flowId, context));
            const prompt = yield this.promptBuilder.buildPromptFor(block, intx);
            if (prompt == null) {
                throw new index_1.ValidationException(`Unable to build a prompt for ${JSON.stringify({
                    context: context.id,
                    intx,
                    block,
                })}`);
