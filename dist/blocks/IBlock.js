...
function findBlockExitWith(uuid, block) {
    ...
    if (exit == null) {
        throw new index_1.ValidationException('Unable to find exit on block');
    }
    return exit;
}
...
function findFirstTruthyEvaluatingBlockExitOn(block, context) {
    const { exits } = block;
    if (exits.length === 0) {
        throw new index_1.ValidationException(`Unable to find exits on block ${block.uuid}`);
    }
...
function findDefaultBlockExitOn(block) {
    ...
    if (exit == null) {
        throw new index_1.ValidationException(`Unable to find default exit on block ${block.uuid}`);
    }
    return exit;
}
...
function generateCachedProxyForBlockName(target, ctx) {
    return new Proxy(target, {
        get(target, prop, _receiver) {
            ...
            const { value } = index_1.findInteractionWith(evalBlock.__interactionId, ctx);
...
function createEvalContextFrom(context) {
    ...
    let prompt;
    if (cursor != null) {
        flow = index_1.getActiveFlowFrom(context);
        block = index_1.findBlockWith(index_1.findInteractionWith(cursor.interactionId, context).blockId, flow);
...
function setContactProperty(block, context) {
    var _a, _b;
    if (index_1.isSetContactPropertyConfig(block.config)) {
