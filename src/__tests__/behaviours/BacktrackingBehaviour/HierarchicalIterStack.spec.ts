  ...
  describe('_append', () => {
    ...
    describe('when first item on subsequent iterations', () => {
      it('should leave head alone', async () => {
        const stack: IStack = createStackFrom([[{uuid: 'abc-123'}, {uuid: 'abc-234'}], []])
        const entity: IEntity = {uuid: 'abc-345'}
  ...
  describe('truncateIterationFrom', () => {
    ...
    describe('when on root stack', () => {
      it('should return removed items; from last key iteration index when non-empty key provided', async () => {
        const stack: IStack = createStack([{uuid: 'abc-123'}, {uuid: 'abc-234'}, {uuid: 'abc-345'}, {uuid: 'abc-456'}, {uuid: 'abc-567'}])

        const expectedStack = createStack([{uuid: 'abc-123'}, {uuid: 'abc-234'}])

        const key: Key = [createStackKey(0, 1)]
        expect(truncateIterationFrom(key, stack)).toEqual([{uuid: 'abc-345'}, {uuid: 'abc-456'}, {uuid: 'abc-567'}])
    ...
    describe('when on nested stack', () => {
      it('should return removed items; from last key iteration index when non-empty key provided', async () => {
        const stack: IStack = createStack([
          createStack([{uuid: 'abc-123'}, {uuid: 'abc-234'}, {uuid: 'abc-345'}, {uuid: 'abc-456'}, {uuid: 'abc-567'}]),
        ])

        const expectedStack = createStack([createStack([{uuid: 'abc-123'}, {uuid: 'abc-234'}])])
  ...
  describe('deepTruncateIterationsFrom', () => {
    ...
    it('should remove items from deepest stack', async () => {
      const stack: IStack = createStack([
        ...
        createStack([{uuid: 'ghi-123'}, {uuid: 'ghi-234'}, {uuid: 'ghi-345'}, {uuid: 'ghi-456'}, {uuid: 'ghi-567'}]),
      ])
