...
describe('IBlock', () => {
  ...
  describe('generateCachedProxyForBlockName', () => {
    ...
    describe('proxy', () => {
      ...
      it(
        'should perform search over interactions for block whose name matches prop name when prop absent from target',
        async () => {
      ...
      it(
        'should perform search over interactions from right-to-left to provide most recent interaction value',
        async () => {
          const ctx = dataset.contexts[1]
          ...
          set(
            ctx,
            'sessionVars.blockInteractionsByBlockName.1570221906056_83',
            {__interactionId: expectedInteractionId})
