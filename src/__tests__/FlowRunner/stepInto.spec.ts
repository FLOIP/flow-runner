...
describe('FlowRunner/stepInto', () => {
  ...
  it(
    'should raise when last interaction doesn\'t match provided blockId (aka only allow step ins during active interaction)',
    async () => {
      const
  ...
  it(
    'should leave run flow interaction\'s selected exit and exitAt empty until we\'ve exited last block in the flow',
    async () => {
      const
        ctx = dataset.contexts[2]
      const block = ctx.flows[0].blocks[0]
